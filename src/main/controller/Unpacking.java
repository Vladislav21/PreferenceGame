...
public class Unpacking implements GameStrategy {
    ...
    @Override
    public void run(Bot bot1, Bot bot2, Bot bot3, Distributor distributor, Logger logger) {
        Random random = new Random();
        int select = random.nextInt(3);
        drawOfCard(select, logger, bot1, bot2, bot3, distributor);
    }
    ...
    private void drawOfCard(int select, Logger log, Bot botMiser, Bot bot1, Bot bot2, Distributor distributor) {
        for (int i = 0; i < 10; i++) {
            log.info("\nХод номер:" + i);
            Card checkCard;
            Card card1;
